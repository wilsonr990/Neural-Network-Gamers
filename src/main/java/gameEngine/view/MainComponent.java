...
public class MainComponent extends JComponent{
    ...
    public MainComponent(GameLoop gameLoop) {
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (gameLoop.displayStatisticsActive) {
            ...
            g.drawString("t = " + Long.toString(gameLoop.world.clock / 1000), 20, 105);
        ...
        if (gameLoop.singleSnakeModeActive) {
            gameLoop.snakes.getFirst().brainNet.display(g, 0, gameLoop.world.width, gameLoop.world.height);
        }
        // snakes:
        synchronized (gameLoop.snakes) {
            for (Snake s : gameLoop.snakes)
                s.draw(g);
            gameLoop.world.draw(g);
        }
    }
}
